...
class SummarizerDocumentCorefResolver(MentionsCorefResolver):
    def __init__(self, mention_tagger, summary_generation_config, **kwargs):
        ...
        self.summarization_pipeline = Text2TextGenerationPipeline(
            ...
            framework="pt",
            generation_config=GenerationConfig(**summary_generation_config['generation']),
        )
        ...
        for (topic, doc_id, sentence_id, sentence, doc), triggers in zip(
            ...
            for i, trigger in enumerate(triggers):
                ...
                mention_map[mention_id] = {
                    ...
                    "marked_doc": doc.replace(sentence, sentence[: start]
                                              + " <m> " + mention_txt + " </m> " + sentence[end:])
                }

        # Summarize mentions
        prompts = [jinja2.Template(SUMMARIZATION_TEMPLATE).render(document=mention_map[mention_id]['marked_doc']) for
                   mention_id in mention_map.keys()]
        summaries = []

        for i in tqdm(range(0, len(prompts), self.batch_size), desc="Summarizing"):
            prompt_batch = prompts[i: i + self.batch_size]
            summaries.extend(output['generated_text'] for output in self.summarization_pipeline(prompt_batch, **kwargs))
